...
- Module repo `versions.tf` had no `popsrox` declaration (added in this PR for fleet alignment). Example `versions.tf` had `popsrox-utils` (hyphenated local name); normalized to `popsrox` to match the rest of the fleet — no consumer-facing `data "popsrox-utils_*"` references existed in this example. `azurerm_security_center_contact`, `azurerm_security_center_workspace`, and `azurerm_security_center_subscription_pricing` are 4.x-compatible without argument changes (verified against the 4.x docs).
